export namespace StringType {
   ...
   */
  export type Split<T extends string, P extends string = ""> = T extends `${infer F}${P}${infer Rest}`
    ? [F, ...Split<Rest, P>]
    : T extends ""
      ? []
      : [T];
   ...
   */
  export type Join<T extends readonly string[], U extends string> = T extends readonly []
    ? ""
   ...
   */
  export type Tail<T extends readonly string[]> = T extends readonly [unknown, ...infer Rest] ? Rest : [];
}
