...
interface NaverStoreProduct {
  ...
  benefits: {
    points: {
      total: number;
      basic: number;
      membershipExtra?: number;
      cardExtra?: number;
      naverpayExtra?: number;
    };
    events: string[];
    gifts: {
      description: string;
...
export namespace NaverStoreCrawlerProvider {
  export const crawl = async (
    ...
    const $ = await CommonExtractor.fetchPage(
      request,
      `[
          { "click": "#content > div > div.z7cS6-TO7X > div._27jmWaPaKy > ul > li:nth-child(2) > a" },
          { "wait_for": "#REVIEW > div > div._2LvIMaBiIO > div._2g7PKvqCKe > ul > li:nth-child(20) > div" }   
      ]`,
    );
  ...
  const extractProductInfo = ($: CheerioAPI): NaverStoreProduct => {
    // 브랜드명 추출
    ...
    const collectPageData = async (
      ...
      return {
        ...
        data: reviews.map((review) => {
          ...
          return {
            text: text,
            images: review.images.map((review) => ({
  ...
  const extractReviewStats = ($: CheerioAPI): ReviewStats => {
    const totalCount =
  ...
  const formatReviewStatusOutput = (stats: ReviewStats): string => {
    const sections = [
