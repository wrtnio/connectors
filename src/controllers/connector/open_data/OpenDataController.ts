...
export class OpenDataController {
   ...
   */
  @RouteIcon("https://ecosystem-connector.s3.ap-northeast-2.amazonaws.com/icon/open_data.svg")
  ...
  ): Promise<Try<IMOLIT.IgetRTMSDataSvcSHRentOutput>> {
    const data = await retry(() => OpenDataProvider.getRTMSDataSvcSHRent(input))();
  ...
  ): Promise<Try<IMOLIT.IGetRTMSDataSvcOffiRentOutput>> {
    const data = await retry(() => OpenDataProvider.getRTMSDataSvcOffiRent(input))();
    return createResponseForm(data);
  }
  ...
  ): Promise<Try<IMOLIT.IGetRTMSDataSvcAptRentOutput>> {
    const data = await retry(() => OpenDataProvider.getRTMSDataSvcAptRent(input))();
    return createResponseForm(data);
  }
  ...
  async getLHLeaseInfo(@TypedBody() input: ILH.IGetLHLeaseInfoInput): Promise<Try<ILH.IGetLHLeaseInfoOutput>> {
    const data = await retry(() => OpenDataProvider.getLHLeaseInfo(input))();
    return createResponseForm(data);
  }
  ...
  async getParkingLot(@TypedBody() input: INIA.IGetParkingLotInput): Promise<Try<INIA.IGetParkingLotOutput>> {
    const data = await retry(() => OpenDataProvider.getParkingLot(input))();
    return createResponseForm(data);
  }
  ...
  async getBuildingInfo(@TypedBody() input: IMOLIT.GetBuildingInfoInput): Promise<Try<IMOLIT.GetBuildingInfoOutput>> {
    const data = await retry(() => OpenDataProvider.getBuildingInfo(input))();
    return createResponseForm(data);
  }
  ...
  ): Promise<Try<IOpenData.MinistryOfTheInteriorAndSafety.IGetStandardRegionCodeListOutput>> {
    const data = await retry(() => OpenDataProvider.getStandardRegionCodeList(input))();
    return createResponseForm(data);
  }
  ...
  ): Promise<Try<IOpenData.FinancialServicesCommission.IGetStockPriceInfoOutput>> {
    const data = await retry(() => OpenDataProvider.getStockPriceInfo(input))();
    return createResponseForm(data);
  }
  ...
  ): Promise<Try<IOpenData.IKoreaMeteorologicalAdministration.IGetVillageForecastInformationOutput>> {
    const data = await retry(() => OpenDataProvider.getShortTermForecast(input))();
    return createResponseForm(data);
  }
  ...
  ): Promise<Try<KoreaCopyrightCommission.IGetCopyRightOutput>> {
    const data = await retry(() => OpenDataProvider.getCopyRight(input))();
    return createResponseForm(data);
  }
}
