import { IMigrateDocument, IMigrateRoute, OpenApi, OpenApiV3 } from "@samchon/openapi";
...
export namespace OpenAiConverter {
  ...
  export const convert = (props: {
    ...
    const migrated: IMigrateDocument = props.migrated ? props.migrated : OpenApi.migrate(props.document);
    ...
    for (const [path, collection] of Object.entries(props.document.paths ?? {}))
      for (const method of Object.keys(collection))
        if (functions.find((v) => v.path === path && v.method === method) === undefined)
    ...
    (route: IMigrateRoute): IOpenAiDocument.IFunction | null => {
      ...
      const parameter = {
        ...
        properties: Object.fromEntries([
          ...
          ...(route.query ? [[route.query.key, escape(route.query.schema)]] : []),
          ...(route.body ? [[route.body.key, escape(route.body.schema)]] : []),
        ]),
      } satisfies OpenApiV3.IJsonSchema.IObject;
      if (Object.values(parameter.properties).some((v) => v === null)) return null;

      const output: OpenApi.IJsonSchema | null | undefined = route.success ? escape(route.success.schema) : undefined;
        ...
        description: (() => {
          if (operation.summary && operation.description) {
            return operation.description.startsWith(operation.summary)
              ...
              : [operation.summary, operation.summary.endsWith(".") ? "" : ".", "\n\n", operation.description].join("");
          }
  ...
  const escapeReference =
    ...
    (input: OpenApi.IJsonSchema): OpenApi.IJsonSchema | null => {
      if (OpenApiTypeChecker.isReference(input)) {
        ...
        const target: OpenApi.IJsonSchema | undefined = components.schemas?.[name];
        ...
        // OBJECT
        const properties: Array<[string, OpenApi.IJsonSchema | null]> | undefined = input.properties
          ? Object.entries(input.properties).map(
              ([key, value]) => [key, escapeReference(components)(visited)(value)] as const,
            )
          : undefined;
        const additionalProperties: OpenApi.IJsonSchema | null | boolean | undefined = input.additionalProperties
          ? typeof input.additionalProperties === "object" && input.additionalProperties !== null
        ...
        return {
          ...
          properties: properties
            ? Object.fromEntries(properties.filter(([_k, v]) => !!v) as Array<[string, OpenApi.IJsonSchema]>)
            : undefined,
          additionalProperties,
        };
      } else if (OpenApiTypeChecker.isTuple(input)) {
        // TUPLE
        const prefixItems: Array<OpenApi.IJsonSchema | null> = input.prefixItems.map((schema) =>
          escapeReference(components)(visited)(schema),
        );
        const additionalItems: OpenApi.IJsonSchema | null | boolean | undefined =
          typeof input.additionalItems === "object" && input.additionalItems !== null
        ...
        // ARRAY
        const items: OpenApi.IJsonSchema | null = escapeReference(components)(visited)(input.items);
