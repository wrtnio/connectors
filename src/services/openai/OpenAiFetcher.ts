...
export namespace OpenAiFetcher {
  ...
  export const request = async (props: IProps) => {
    ...
    return PlainFetcher.fetch(
      props.connection,
      {
        ...
        path:
          props.document.isKeywordParameter === true
            ? getKeywordPath({
  ...
  const getKeywordPath = (props: { function: IOpenAiDocument.IFunction; input: Record<string, any> }): string => {
    const route: IMigrateRoute = props.function.route();
    let path: string = route.emendedPath;
    for (const p of route.parameters) path = path.replace(`:${p.key}`, props.input[p.key] ?? "null");
    if (route.query) path += getvariable(props.input.query ?? {});
    return path;
  };

  const getPositionalPath = (props: { function: IOpenAiDocument.IFunction; arguments: any[] }): string => {
    const route: IMigrateRoute = props.function.route();
    let path: string = route.emendedPath;
    route.parameters.forEach((pp, i) => {
      path = path.replace(`:${pp.key}`, props.arguments[i]);
    });
    if (route.query) path += getvariable(props.arguments[route.parameters.length]);
    return path;
  };
