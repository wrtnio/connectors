...
export const test_studio1_chatbot_easy_generate = async (connection: CApi.IConnection) => {
  const easyInput: IChatbot.IChatbotEasyGenerateInput = {
    ...
    requirement: "사용자의 입력에 따라 유머나 농담을 적절하게 반영해줘야 합니다.",
    histories: [
      { role: "user", content: "내 기분을 풀어줘" },
      { role: "assistant", content: "우하하하, 제가 기분을 풀어드리죠!" },
    ],
  };
  const output = await CApi.functional.connector.chatbot.generate.easy.generateEasyChatbot(connection, easyInput);
